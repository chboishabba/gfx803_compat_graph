#!/usr/bin/env bash
# Wrapper to run the cloned 6.4-upgrade Python environment on the host.
# This keeps the upgraded lane separate from the control 6.4 runtime.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(dirname "$SCRIPT_DIR")"
ARTIFACT_DIR="${ROCM64_UPGRADE_OUTDIR:-$REPO_ROOT/artifacts/rocm64-upgrade}"
...
if [ ! -d "$COMPAT_DIR" ] || [ -z "$PYTHON_BIN" ]; then
  echo "ERROR: 6.4-upgrade compatibility libraries or python environment not present." >&2
